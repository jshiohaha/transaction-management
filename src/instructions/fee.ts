...
/**
 * Derives the prioritization fee based on the given fee configuration.
 *
 * @param connection - The Solana connection object.
 * @param transaction - The Solana transaction object.
 ...
 */
export const derivePrioritizationFee = async (
    connection: Connection,
    instructions: TransactionInstruction[],
    ...
    if (feeConfig?.type === "constant") {
        ...
        feeConfig?.type === "dynamic" &&
        feeConfig?.calculatePriorityFee
    ) {
...
export const generatePrioritizationFeeInstruction = async (
    ...
    derivePrioritizationFee(connection, instructions, feeConfig).then((fee) =>
